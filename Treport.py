...
class Treport:
    def __init__(self, path):
        book = xlrd.open_workbook(path)  # 報告書データを開く
        sheets = book.sheets()
        self.s = sheets[0]
    ...
    def delete_unnecc(self, i):  # 不必要な文字を消したり読み取れない文字を置き換えたりしている
        noenc = self.s.cell_value(i, 3).replace(u'－', u'-')  # cell_value(i,3)--各行の3列目は報告書本文で扱えない文字を置き換えている
    ...
    if __name__ == '__main__':
        ...
        for N in list(Series(Noun_uniq)[Noun_uniq2 == "No entry"]):
            ...
            if re.search("[0-9]", N_uni):
                numin = True
            if re.search("[{-~]", N_uni) or re.search("[[-`]", N_uni) or re.search("[ -/]", N_uni) or re.search("[:-@]",
                                                                                                                N_uni):
                alpha = True
        ...
        for V in list(Series(Verb_uniq)[Verb_uniq2 == "No entry"]):
            ...
            if re.search("[0-9]", V_uni):
                numin = True
            if re.search("[{-~]", V_uni) or re.search("[[-`]", V_uni) or re.search("[ -/]", V_uni) or re.search("[:-@]",
                                                                                                                V_uni):
                alpha = True    
